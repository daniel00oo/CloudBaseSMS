...
class Runner(object):
	...
	def __init__(self, loadFile):
		"""
			...
			Variables:
				...
				conv - instance of Converter class, used to convert json files to python dictionary and vice-versa
				commands - python dictionary, used to store the path to the commands of all operating systems
				osFormat - python dictionary, used to store the necessary formating for the files
		...
		#dictionary of shortcuts to actual files {"Operating system" : ["path", "to", "commands"]}
		#access a command: 
		#	r = Runner(filePath)
		...
		for operatingSystem in self.commands:
			#for portability, we use os.path.join to join the path with the correct OS separator
			self.commands[operatingSystem] = os.path.join(*self.commands[operatingSystem])	
	...
	def run(self, operatingSystem):
		...
		if not os.path.isdir('metrics'):
			os.mkdir('metrics')	#making directory to store the metrics to look tidy
