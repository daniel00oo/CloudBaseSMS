...
class Receive(Receiver.Receiver):
	"""
		Extention of Receive class made custom to automatically start a process
		to collect and store metrics of host unit

		r = Receive([url, dbhost])
			url - url string, the url in from which to receive messages via RabbitMQ
		...
		Variables:
			osys - string, name of the host operating system 
				(e.g.: 'Windows')
			runner - instance of Runner class, used to run files to get metrics
			storage - instance of StorageMongoDB class, used to store the metrics into a noSQL database
				(using pymongo package and MongoDB)
			conv - instance of Converter class, used to convert json files into python dictionaries
			t - instance of Repeater class, used to repeat the '__run' task at given intervals
			is_running - boolean value, describes if the process is running or not 
				(changes with 'start' and 'stop' methods)

		Methods:
			howToProcess(self, body) - method used to process the received message
			__run - used to run the files and commands that will get and store the desired metrics into a database
			start - used to start the process, calls the '__run' method.
			stop - used to stop the process called by the 'start' method.
	...
	def __run(self):
		...
		d = self.conv.fromJSONtoDict(os.path.join(
			...
			self.osys, 
			'metrics', 
			'metrics.json'))	#converting tools/Linux/metrics/metrics.json for example
...
if len(sys.argv) == 2 and sys.argv[1].lower() == 'help':
	...
	if len(sys.argv) >= 4:
		...
		database = sys.argv[3]
	if len(sys.argv) >= 3:
		#the 2nd argument becomes the RabbitMQ url (from which messages will be extracted)
		url = sys.argb[2]
	if len(sys.argv) >= 2:
		#the 1st argument becomes the queue name from which to extract messages via RabbitMQ
		queue = sys.argv[1]
